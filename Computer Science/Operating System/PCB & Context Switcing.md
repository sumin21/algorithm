...
> os가 프로세스를 제어하기 위해, 프로세스 메타데이터들을 저장해 놓는 곳, 한 PCB 안에는 한 프로세스의 정보가 담김
> 
> = 프로세스 상태정보를 저장하는 구조체.
> 
> 프로세스 생성 관리와 Context Switching을 위해 필요
...
> 일반적으로 사용자 메모리와는 별도로 운영 체제의 커널 메모리에 저장된다. 
> 
> 커널은 PCB 테이블을 유지 관리하며 테이블의 각 항목은 특정 프로세스에 해당합니다. 커널은 PCB의 정보를 사용하여 프로세스의 실행, 스케줄링 및 리소스 할당을 관리합니다.
